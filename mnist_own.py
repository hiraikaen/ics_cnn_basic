...
# grid = make_grid(torch.Tensor((train.iloc[random_sel, 1:].as_matrix()/255.).reshape((-1, 28, 28))).unsqueeze(1), nrow=8)
...
class MNIST_data(Dataset):
    ...
    def __init__(self, file_path, 
        ...
        if len(df.columns) == n_pixels:
            # test data
            self.X = df.iloc[:,1:].values.reshape((-1,SIZE_X,SIZE_Y)).astype(np.uint8)[:,:,:,None]
...
def evaluate(data_loader):
    ...
    print('\nAverage loss: {:.4f}, Accuracy: {}/{} {:.4}%'.format(
